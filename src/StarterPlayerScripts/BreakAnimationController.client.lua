--[=[
	...
	The shatter is delayed by the interpolation delay so it lines up with where the
	Bey is actually being rendered.
]=]
...
local function shatter(model)
	...
	for _, part in ipairs(model:GetDescendants()) do
		if part:IsA("BasePart") then
			...
			part.CanQuery = false
			-- Fling outward from the centre, with an upward kick and random spin.
...
local function onBreak(pid, position)
	...
	task.delay(Constants.InterpolationDelay, function()
		local model = workspace:FindFirstChild("Bey_" .. tostring(pid))
		local pos = (model and model:GetPivot().Position) or position or Vector3.new(0, Constants.BeyRadius, 0)
		doFlash()
		sparkBurst(pos)
		shatter(model)
	end)
...
Remotes.StateSnapshot.OnClientEvent:Connect(function(snapshot)
	...
	for _, ev in ipairs(snapshot.events) do
		if ev.eventType == "HpBreak" then
			onBreak(ev.eventData.playerId, ev.eventData.position)
		end
	end
end)
