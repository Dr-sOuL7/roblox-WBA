--[=[
	...
	Real-time analog BATTLE control: a virtual joystick (sets facing) plus DASH and
	REVOLVE hold-buttons, mirrored on the keyboard. Sends a compact input packet
	{ facingAngle, dash, revolve } to the server ~15 Hz.

	The controls are shown only during the Active battle phase (the launch ceremony
	and hub walking use their own UI / the default character controls).

	Joystick is world-relative for the top-down/spectator camera: pushing RIGHT aims
	the Bey +X, DOWN aims +Z. Dash and Revolve may be held together (combo = revolve
	at 3× speed). The server only applies facing when neither ability is held, and
	gates abilities on Mana — this is purely intent.
]=]
...
local keyVec = { x = 0, z = 0 } -- keyboard direction held-set

-- ── GUI (hidden until the battle is Active) ─────────────────────────────────────
...
-- ── Phase tracking (show controls only in battle) ───────────────────────────────
Remotes.MatchStateChanged.OnClientEvent:Connect(function(phase)
	currentPhase = phase
	gui.Enabled = (phase == "Active")
